...
import me.lolico.pms.exception.BussException;
...
public class TBugServiceImpl implements ITBugService {
    ...
    public int insertTBug(TBug tBug) {
        ...
        requirement.setId(tBug.getRequirementNo());
        List<TRequirement> requirements = tRequirementMapper.selectTRequirementList(requirement);
        if (requirements.isEmpty()) {
            throw new BussException("需求编号不存在");
        }
